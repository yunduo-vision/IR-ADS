...
def main(cfg, gpu, save_dir):
    ...
    gpus = 1#int(os.environ['WORLD_SIZE'])
    ...
    extra_pretrained_model = {}
    for k, v in model_param.items():
        if 'D_fc1.weight' in k:
            nn.init.kaiming_uniform_(v, a=math.sqrt(5))
            extra_pretrained_model[k] = v
        if 'D_fc2.weight' in k and 'MPG' in k:
            nn.init.kaiming_uniform_(v, a=math.sqrt(5))
            extra_pretrained_model[k] = v
        if 'D_fc2.weight' in k and 'Adapter' in k:
            nn.init.zeros_(v)
            extra_pretrained_model[k] = v
        if 'patch_embed' in k and 'extra_' not in k:
            k = k.split('.')
            k[1] = 'extra_' + k[1]
            k = '.'.join(k)
            extra_pretrained_model[k] = v
        if 'norm' in k and 'block' not in k and 'extra_' not in k and 'stages' not in k:
            k = k.split('.')
            k[1] = 'extra_' + k[1]
            k = '.'.join(k)
            extra_pretrained_model[k] = v
    model.load_state_dict({k: v for k, v in extra_pretrained_model.items()}, strict=False)
    ...
    start_epoch = 0
    optimizer = get_optimizer(model, optim_cfg['NAME'], lr, optim_cfg['TRAIN_TYPE'], optim_cfg['WEIGHT_DECAY'])
    ...
    for epoch in range(start_epoch, epochs):
        ...
        for iter, (sample, lbl) in pbar:
            ...
            with autocast(enabled=train_cfg['AMP']):
                logits, logits_rgb, logits_dte = model(sample)
                ...
                loss1 = loss_fn(logits, lbl)

                # MMST
                loss4 = loss_fn(logits_rgb, mask_lbl)
                loss5 = loss_fn(logits_dte, mask_lbl)
                loss = loss1 + 0.01 * loss4 + 0.01 * loss5
