...
class DINOCriterion(TwoStageCriterion):
    ...
    def compute_dn_loss(self, dn_metas, targets, aux_num, num_boxes):
        ...
        if dn_metas and "output_known_lbs_bboxes" in dn_metas:
            ...
            losses["loss_roi_dn"] = torch.as_tensor(0.0).to("cuda")
            losses["loss_mask_dn"] = torch.as_tensor(0.0).to("cuda")
            losses["loss_dice_dn"] = torch.as_tensor(0.0).to("cuda")
        ...
        for i in range(aux_num):
            ...
            if dn_metas and "output_known_lbs_bboxes" in dn_metas:
                ...
                l_dict["loss_roi_dn"] = torch.as_tensor(0.0).to("cuda")
