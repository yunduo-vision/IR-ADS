...
class SUN(Dataset):
    ...
    def __init__(self, root: str = 'data/SUNRGBD', split: str = 'train', transform=None, modals=['img', 'depth'],
                 case=None) -> None:
        super().__init__()
    ...
    def __getitem__(self, index: int) -> Tuple[Tensor, Tensor]:
        ...
        x1 = os.path.join(*[self.root, 'Depth', item_name + '.png'])
    ...
    def _open_img(self, file):
        ...
        img = torch.tensor(np.array(cv2.imread(file, cv2.IMREAD_COLOR))).permute(2,0,1)
        C, H, W = img.shape
        if C == 4:
            img = img[:3, ...]
        if C == 1:
    ...
    def _get_file_names(self, split_name):
        ...
        for item in files:
            ...
            if ' ' in file_name:
                file_name = file_name.split(' ')[0].split('/')[1].split('.')[0]
