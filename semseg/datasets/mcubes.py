...
class MCubeS(Dataset):
    ...
    num_classes: 20
    """
    CLASSES = ['asphalt', 'concrete', 'metal', 'road_marking', 'fabric', 'glass', 'plaster', 'plastic', 'rubber',
               'sand',
    ...
    def __init__(self, root: str = 'data/MCubeS/multimodal_dataset', split: str = 'train', transform=None,
                 modals=['image', 'aolp', 'dolp', 'nir'], case=None) -> None:
        super().__init__()
        ...
        self.base_size = 512
        self.crop_size = 512
        self.files = ['outscene1208_7_0000000415', 'outscene1208_7_0000000445', 'outscene1209_16_0000000080',
                      'outscene1210_11_0000001410']  # self._get_file_names(split)
    ...
    def __getitem__(self, index: int) -> Tuple[Tensor, Tensor]:
        ...
        img = img.astype(np.float32) / 65535 if img.dtype == np.uint16 else img.astype(np.float32) / 255
        img = np.array(img).astype(np.float32).transpose((2, 0, 1))
        img = torch.from_numpy(img)
        _target = cv2.imread(lbl_path, -1)
        label_ori = cv2.imread(lbl_path, -1)
        label_ori = np.array(label_ori).astype(np.float32)
        ...
        _img, _target, _aolp, _dolp, _nir, _nir_mask, _mask = _img[:, self._left_offset:], _target[:,
                                                                                           self._left_offset:], \
        ...
        sample = {'image': _img, 'label': _target, 'aolp': _aolp, 'dolp': _dolp, 'nir': _nir, 'nir_mask': _nir_mask,
                  'u_map': self.u_map, 'v_map': self.v_map, 'mask': _mask}
    ...
    def _get_file_names(self, split_name):
        ...
        source = os.path.join(self.root, 'list_folder/test.txt') if split_name == 'val' else os.path.join(self.root,
