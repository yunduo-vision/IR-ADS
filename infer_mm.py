...
class SemSeg:
    ...
    def predict(self, img_fname: str, overlay: bool) -> Tensor:
        if cfg['DATASET']['NAME'] == 'DELIVER':
            ...
            x1 = img_fname.replace('/rgb', '/ther')
            # x2 = img_fname.replace('/img', '/lidar').replace('_rgb', '_lidar')
            # x3 = img_fname.replace('/img', '/event').replace('_rgb', '_event')
            lbl_path = img_fname.replace('/rgb', '/labels')
...
if __name__ == '__main__':
    ...
    if test_file.is_file():
        ...
        if cfg['DATASET']['NAME'] == 'DELIVER':
            ...
            source = os.path.join(test_file, 'test.txt')
            files = []
            with open(source) as f:
                files_ = f.readlines()
            for item in files_:
                ...
                files.append(os.path.join('data/MFNet/rgb', file_name+'.png'))
        else:
